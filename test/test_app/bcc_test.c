...
/*	result = ioctl(Handle, BCC_SET_MK2_MODE, NULL);	
	...
	result = ioctl(Handle, BCC_GET_SERIAL_NUMBER_LENGTH, &data);	
	if (result == -1) {
...
/*	else 
	...
	if (data != 0) {
		Serial = (char*) malloc(data * sizeof(char)); 				
...
/*	result = ioctl(Handle, BCC_GET_INCREMENT_ENABLE, &data);	
	...
	for (i=1 ;i <WORDTEST; i++) {
		buffer[i] = (i - 1); }		
		buffer[0] = 0;	
										
	result = write(Handle, &buffer, WORDTEST);
	if(result == -1)
		printf("Unable to write to device bccif1 with error - %s\n", strerror(errno));
	else
	...
	buffer[0] = 0;
	result = write(Handle, &buffer[0], 1);								//Set the address back to be beginning
	result = read(Handle, &buffer, WORDTEST - 1);		
	if(result == -1)
		printf("Unable to read from device bccif1 with error - %s\n", strerror(errno));
	else
		for(i=0; i< result; i++){
		if (buffer[i] != i) {
			...
			break;}}
		if (i == WORDTEST - 1) printf("Read completed OK, read %d words correctly out of %d \n", i, WORDTEST - 1);			
...
//			printf("Write completed OK, wrote %d words\n", result); 		 
		...
		result = read(Handle, &buffer[1], 1);
		if(result == -1)
			printf("Unable to read from device bccif2 with error - %s\n", strerror(errno));		
		
		if (buffer[1] != buffer[0] || buffer[0] != i || buffer[1] != i){
